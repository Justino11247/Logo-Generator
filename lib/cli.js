...
// Error Handling to prevent text input from being more then 3 in length
  ...
  renderLogo(shape, shapeColor, text, letterColor, filename) {
    try {
      if (typeof shape === 'object' && 'setColor' in shape) {
        shape.setColor(shapeColor);
        
        // Get shape-specific text positioning
        const { x, y, fontSize } = shape.getTextPosition();
        
        const svgContent = `<svg version="1.1" width="300" height="200" xmlns="http://www.w3.org/2000/svg">
          ${shape.render()}
          <text x="${x}" y="${y}" font-size="${fontSize}" text-anchor="middle" fill="${letterColor}">${text}</text>
        </svg>`;
        
        const filePath = `./examples/${filename}.svg`;
        
        writeFile(filePath, svgContent, 'utf-8')
          .then(() => {
  ...
  async run() {
    ...
    return inquirer.prompt(questions)
      .then((answers) => {
        ...
        const filename = answers.text.toLowerCase().replace(/[^a-z0-9]/g, '_');
        this.renderLogo(shape, answers.shapeColor, answers.text, answers.textcolor, filename);
      });
  }
  }
